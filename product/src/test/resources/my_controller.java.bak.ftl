...
<#assign eservice = entity?uncap_first + 'Service' >
	...
	public Result<List<${entity}>> list(){
		...
		List<${entity}> list = ${eservice}.list(wrapper);
	...
	@GetMapping("/load/{id}")
	@ApiOperation("加载${table.comment!}")
	public Result<${entity}> load(@PathVariable @ApiParam(required = true) String id){
		${entity} entity = ${eservice}.getById(id);
